...
class PharmacyMonthYearExpiryField extends Component {
    ...
    setup() {
        ...
        this.state = useState({ displayValue: this._formatMonthYear(this._getValue(this.props)) });
    ...
    _formatMonthYear(value) {
        if (!value) return "";
        if (typeof value === "string" && /^(0[1-9]|1[0-2])\/\d{4}$/.test(value)) return value;
        if (value.toFormat) return value.toFormat("MM/yyyy");
    ...
    onInput(ev) {
        this.state.displayValue = ev.target.value;
    }
    ...
    async onChange(ev) {
        ...
        if (!match) {
            this.notification.add(_t("Expiry date must be in MM/YYYY format, for example 04/2026."), { type: "danger" });
        ...
        const newValue = fieldType === "datetime"
            ? lastDay.set({ hour: 12, minute: 0, second: 0, millisecond: 0 })
            : lastDay.startOf("day");
        await this.props.record.update({ [this.props.name]: newValue });
        this.state.displayValue = newValue.toFormat("MM/yyyy");
    }
}
